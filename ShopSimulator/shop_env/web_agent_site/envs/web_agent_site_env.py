...
from web_agent_site.engine.options import format_option_action
...
class WebAgentSiteEnv(gym.Env):
    ...
    def get_available_actions(self):
        ...
        for opt in buying_options:
            if opt.is_selected():
                continue
            opt_axis = opt.get_attribute('name')
            opt_value = opt.get_attribute('value')
            self.text_to_clickable[
                format_option_action(opt_axis, opt_value)
            ] = opt
        return dict(
            has_search_bar=has_search_bar,
            clickables=[
                value
                for value in self.text_to_clickable.keys()
                if value.casefold() != "search"
            ],
        )
